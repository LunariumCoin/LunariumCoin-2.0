...
{
    if (nHeight >  2442400) return 500000 * COIN;
    if (nHeight >  1916800) return 450000 * COIN;
    if (nHeight >  1391200) return 400000 * COIN;
    if (nHeight >   865600) return 350000 * COIN;
    if (nHeight >   340000) return 300000 * COIN;
...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    // XLN coin supply for distribution
    if (nHeight == consensus.nXLNCoinSupplyMintHeight) return consensus.nXLNCoinSupply + GetBlockValue(nHeight + 1);

    if (nHeight >  1916800) return       12 * COIN;
    if (nHeight >  1391200) return       12 * COIN;
    if (nHeight >   865600) return       12 * COIN;
    if (nHeight >   340000) return       12 * COIN;
    if (nHeight >    14400) return       12 * COIN;
    if (nHeight >        1) return        1 * COIN;
    if (nHeight >        0) return 51000000 * COIN;
...
CAmount CMasternode::GetMasternodePayment(int nHeight)
{
    ...
    // XLN coin supply for distribution
    if (nHeight == consensus.nXLNCoinSupplyMintHeight) return GetMasternodePayment(nHeight + 1);

    
    if (nHeight >    250) return GetBlockValue(nHeight) * 75 / 100;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 XLN tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                   // block for 1000 XLN tx -> 1 confirmation
